...
''' This will show all the views for the State objects'''
...
def toGetid():
    '''This funciton is the Update fucntion '''
...
@app_views.route('/states/', methods=['POST'],
        strict_slashes=False)
def posting():
    '''This funciton is the post funciton '''
...
@app_views.route('/states/<state_id>', methods=['PUT'],
        strict_slashes=False)
def putinV():
    ''' This fucntion will be used to update '''
    response = request.get_json()
    if response id None:
        abort(400, {'Not a JSON'})
...
@app_views.route('/states/<state_id>', methods=['DELETE'],
        strict_slashes=False)
def deleting():
    ''' This funciton is used to delete '''
    stateObject = storage.get(State, state_id)
    if stateObject is None:
        abort(404)
